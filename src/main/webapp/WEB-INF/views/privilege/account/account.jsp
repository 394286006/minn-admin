...
<input id="menu_title" type="hidden" value=<sp:message code="account_title"/> >
    ...
    <div>
     ...
     <div style="float:left">
       ...
       <div style="width:150px;float:left">  
         <input type="button" value="<sp:message code="account_save_account_resource"/>" onclick="save_account_role()">
	...
	<tr><td><input type="hidden" id="id" name="id"></td></tr>
	  <tr><td><sp:message code="account_name"/>:</td><td><input type="text" id="name" name="name"></td></tr>
	  <tr><td><sp:message code="account_pwd"/>:</td><td> <input type="text" id=pwd name="pwd"></td></tr>
	  <tr><td><sp:message code="account_type"/>:</td><td><select id="type" name="type">
         </select></td></tr>
	  <tr><td><sp:message code="account_logintype"/>:</td><td><select id="logintype" name="logintype">
         </select></td></tr>
...
$(function() {
    ...
    $(".flexdata_grid").flexigrid({
        url : sys_ctx_p+"/account?method=query&column="+columnstr, 
        dataType : 'json',
        colModel : [{
            display : 'ID',
            name : 'id',
            ...
            display : '<sp:message code="account_name"/>',
            name : 'name',
            width : 90,
            sortable : true,
            align : 'center'
            }, {
                display : '<sp:message code="account_type"/>',
                name : 'type_name',
                width : 120,
                sortable : true,
                align : 'left'
            },  {
                display : '<sp:message code="account_logintype"/>',
                name : 'logintype_name',
                width : 120,
                sortable : true,
                align : 'left'
        ...
        sortorder : "asc",
        usepager : true,
        title : '<sp:message code="account_title"/>',
        useRp : true,
        singleSelect: true,
        rp : 20,
        showToggleBtn : false,
    ...
    $(".flexdata_grid").click(function(e,grid){
    	if($('.trSelected', grid).length>0){
    	  ...
    	  selected_account_id=row[0].innerText;
    	  if(selected_account_id==''||selected_account_id==undefined||selected_account_id=='undefined'){
    		  selected_account_id=row[0].textContent;
    	  }
    	  get_account_role(selected_account_id);
    	}
    });
  ...
  //删除
   function del(id){
	   if(confirm("你确定要删除该数据吗!") == true){
		   $.post(sys_ctx_p+'/account?method=del', {id:id }
	       , function(data){
	    	   var d=eval(data);
	    	   if(d.success==true){
  ...
  //修改
    function edit(){
    	  $.post(sys_ctx_p+'/account?method=edit', { id:$('#id').val(),name: $('#name').val(),active:$('#active').val(),pwd:$.md5($('#pwd').val()),type:$('#type').val(),loginType:$('#logintype').val() }
	       , function(data){
	    	   var d=eval(data);
	    	   if(d.success==true){
	    		   alert('Processing is complete！');
  ...
  //保存
	function save(){
		    ...
		    $.post(sys_ctx_p+'/account?method=save', {name: $('#name').val(),active:$('#active').val(),pwd:$.md5($('#pwd').val()),type:$('#type').val(),loginType:$('#logintype').val()}
	       , function(data){
	    	   var d=eval(data);
	    	   if(d.success==true){
	    		   alert('Processing is complete！');
...
//树数据 
 function get_account_role(u_id){
	 $('#menu_sub_sys_div').empty();
	 $('#menu_sub_sys_div').removeAttr('class');
	 $('#menu_sub_sys_div').removeAttr('role');
	 $.ajax({url: sys_ctx_p+"/account?method=getAccountRole",data:{accountid:u_id},dataType: 'json'}).done(function( data ) {
		  var d=eval(data);
		  if(d.success==true){
...
$.ajax({url: sys_ctx_p+"/dic?method=getDic",data:{type:"'ACCOUNTTYPE','ACTIVETYPE','LOGINTYPE'"},dataType: 'json'}).done(function( data ) {
 ...
 function save_account_role(){
	 ...
	 if(selected_account_id==''){
	  ...
	  $.post(sys_ctx_p+'/account?method=saveAccountRole', {  accountid:selected_account_id,roleids:role_ids  }
      , function(){
    	  alert('Processing is complete！');
  });
 }
